...
def preProcessTextInDataFrame(df):
    ...
    for idx, row in df.iterrows():
        ...
        for sentence in sentences:
            temp_row = row
            temp_row[0] = sentence
            texts_labels_df = texts_labels_df.append(
                temp_row, ignore_index=True)
...
def backTranslate(translator, txt, lang):
    ...
    if translation.text == "" or translation.text == None or len(translation.text) > 5000:
        return txt
    translation_to_vi = translator.translate(
        translation.text, src=lang, dest='vi')
...
def trainData():
    ...
    texts_labels_dummy_df = pd.get_dummies(data=texts_labels_df, columns=[
                                           "Labels"], prefix="", prefix_sep="")
